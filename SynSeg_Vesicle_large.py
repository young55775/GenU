...
bg_cut = 1
model = UNet()
model.load_state_dict(torch.load('SynSeg_Vesicle_seg_larger_vesicles.pth', map_location=torch.device('cpu'))) #16-bit input
model.eval()
img = cv2.imread(r"C:\Users\guozh\Desktop\segmentation\v2.tif",cv2.IMREAD_UNCHANGED).astype('float32') # We recommend padding to at least 512*512 with black pixels.
...
plt.imshow(mask>0.6,cmap='gray') #adjust this threshold for best performance
plt.show(block=True)
mask = np.array(mask)
